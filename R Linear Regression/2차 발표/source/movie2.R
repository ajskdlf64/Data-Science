...
df_train %>% ggplot(aes(x=Runtime, y=Audience)) + 
  geom_point(size=3) + 
  geom_smooth(data=filter(df_train,Audience<10000000),aes(col="outlier"),se=FALSE,method="lm",size=2) + 
  geom_smooth(aes(col="lm"),se=FALSE,method="lm",size=2) + 
  geom_text(data=filter(df_train,Audience>10000000),aes(label=Name),nudge_x=15,size=3) + 
  labs(col="method")
...
df_train %>% ggplot(aes(x=Grade, y=Audience, fill=Grade)) + 
  geom_boxplot() + 
  labs(x="등급", y="관객 수")
...
# Trailer_views
summary(df_train)
df_train %>% ggplot(aes(x=Trailer_views,y=Audience)) + 
  geom_point(size=4) + 
  geom_text(data=filter(df_train,Trailer_views>3000000),aes(label=Name),size=10, nudge_x=-500000, col="Steelblue")
...
library(car)
influencePlot(fit.2)
df_train %>% filter(Audience>5000000) %>% summarise(n=n())
...
# 잠정 모형
fit.2.1 <- lm(log(Audience, base = 10) ~ Month + Genre + Grade + Critic_Reviews + Screen, data=df_train)
fit.2.2 <- lm(log(Audience, base = 10) ~ Critic_Reviews + Screen, data=df_train)
fit.2.3 <- lm(log(Audience, base = 10) ~ Runtime + Grade + Critic_Reviews + Screen, data=df_train)
AIC(fit.2.1) ; AIC(fit.2.2) ; AIC(fit.2.3)
BIC(fit.2.1) ; BIC(fit.2.2) ; BIC(fit.2.3)
summary(fit.2.1)
summary(fit.2.2)
summary(fit.2.3)
...
# 잠정 모형 비교
model <- data.frame(model=c("M1","M2","M3"), AIC=c(9.47,-13.07,-15.37), BIC=c(93.59,-1.85,7.05),R2=c(0.8376,0.8384,0.8464))
model %>% ggplot(aes(x=model,y=AIC,fill=model)) + 
  geom_bar(stat="identity") + 
  geom_text(aes(label=AIC),size=7)
model %>% ggplot(aes(x=model,y=BIC,fill=model)) + 
  geom_bar(stat="identity") + 
  geom_text(aes(label=BIC),size=7)
model %>% ggplot(aes(x=model,y=R2,fill=model)) + 
  geom_bar(stat="identity") + 
  geom_text(aes(label=R2),size=7)


# fit.2.2 가정 만족 확인
fit.2.2 <- lm(log(Audience, base = 10) ~ Critic_Reviews + Screen, data=df_train)
spreadLevelPlot(fit.2.2)
...
# 제곱합을 추가하고 변수 선택
library(leaps)
fits <- regsubsets(log(Audience,base=10) ~ Distributor + Month + Country + Genre + Runtime + 
...
# AIC, BIC 기준에 따른 변수 선택
fit.10 <-lm(log(Audience,base=10) ~ Distributor + Month + Country + Genre + Runtime + 
                     Grade + Screen_Type + Story + Series + Audience_Reviews + Critic_Reviews + 
                     Trailer_views + Screen + I(Screen^2), df_train)
step(fit.10, direction="both", trace=FALSE)
step(fit.10, k=log(nrow(df_train)),trace=FALSE)


# 잠정 모형
fit.2.4 <- lm(log(Audience, base = 10) ~ Month + Genre + Grade + Critic_Reviews + Screen + I(Screen^2), data=df_train)
fit.2.5 <- lm(log(Audience, base = 10) ~ Critic_Reviews + Screen + I(Screen^2), data=df_train)
fit.2.6 <- lm(log(Audience, base = 10) ~ Runtime + Grade + Screen_Type  + Critic_Reviews + Screen + I(Screen^2), data=df_train)
AIC(fit.2.4) ; AIC(fit.2.5) ; AIC(fit.2.6)
BIC(fit.2.4) ; BIC(fit.2.5) ; BIC(fit.2.6)
summary(fit.2.4)
summary(fit.2.5)
summary(fit.2.6)
...
# 잠정 모형 비교
model <- data.frame(model=c("M4","M5","M6"), AIC=c(-2.3,-20.54,-26.33), BIC=c(84.61,-6.52,7.31),R2=c(0.8376,0.8384,0.8492))
model %>% ggplot(aes(x=model,y=AIC,fill=model)) + 
  geom_bar(stat="identity") + 
  geom_text(aes(label=AIC),size=7)
model %>% ggplot(aes(x=model,y=BIC,fill=model)) + 
...
# fit.2.5 가정 만족 확인
plot(fit.2.5, which=1)
plot(fit.2.5, which=2)
forecast::checkresiduals(fit.2.5)
crPlots(fit.2.5)
residualPlots(fit.2.5)


# M2 vs M5 모형 비교
AIC(fit.2.2) ; AIC(fit.2.5)
BIC(fit.2.2) ; BIC(fit.2.5)


# 예측
str(df_test)
pred <- data.frame(10^predict(fit.2.5, newdata=df_test, interval="confidence", level=0.95))
...
# 시각화
result <- data.frame(id=1:30,pred=pred$fit, real=df_test$Audience, name=df_test$Name, error=df_test$Audience-pred$fit)
